...
from L13 import cifar10_conv_L13
...
with tf.variable_scope('model'):
    output = cifar10_conv_L13(X, keep_prob=keep_prob)
...
cifar10_var = [var for var in tvar if 'cifar10_conv_L13' in var.name]
...
for i in range(10000):
    ...
    accuracy_count += 1
    if i % 10 == 0:
        print('time: %f accuracy:%f (hidden layer: 13, keep prob: 0.4, without augmentation) ' % (float(time.time()-t),test_accuracy/accuracy_count))
        t = time.time()
